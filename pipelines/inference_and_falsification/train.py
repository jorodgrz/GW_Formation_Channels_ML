...
from pipelines.data_alignment import (
...
from pipelines.inference_and_falsification.inference.sbi_framework import (
    PopulationSynthesisPrior,
    SBIFramework,
)
from pipelines.inference_and_falsification.models.physics_informed_nn import (
    PhysicsInformedEnsembleModel,
    build_model,
)
...
def create_real_dataloaders(config: Dict) -> Tuple[DataLoader, DataLoader]:
    """
    Instantiate dataloaders backed by real GWTC-4 events and ensemble slices.
    """
    gwtc_cfg = config["gwtc4"]
    dataset_cfg = config.get("dataset", {})

    combined_path = Path(gwtc_cfg["data_path"]).expanduser()
    raw_dir = combined_path.parent if combined_path.suffix else combined_path

    # Aggregate any newly downloaded event files into the combined store
    build_combined_posterior_file(raw_dir=raw_dir, output_file=combined_path)

    loader = GWTC4Loader(
        data_path=str(combined_path),
        catalog_file=gwtc_cfg.get("catalog_file"),
        cosmology_model=gwtc_cfg.get("cosmology", "Planck18"),
        raw_event_dir=str(raw_dir),
    )
    events = loader.load_all_events(
        max_events=gwtc_cfg.get("max_events"),
        bbh_only=gwtc_cfg.get("bbh_only", True),
    )
    if not events:
        raise RuntimeError(
            "No GWTC-4 events available. Download posteriors with download_gwtc4_data."
        )

    ensemble_dirs = _collect_ensemble_dirs(config)
    if not ensemble_dirs:
        logger.warning("No ensemble directories found; code encodings will be zeros.")

    dataset = EventEnsembleDataset(
        events=events,
        code_order=dataset_cfg.get("code_order", config["multi_code"]["codes"]),
        ensemble_dirs=ensemble_dirs,
        input_dim=config["model"]["input_dim"],
        obs_dim=config["model"]["obs_dim"],
        slice_size=dataset_cfg.get("slice_size", 256),
        synthetic_multiplier=dataset_cfg.get("synthetic_multiplier", 1.0),
        include_simulation=dataset_cfg.get("include_simulation", True),
        max_systems_per_code=dataset_cfg.get("max_systems_per_code"),
        seed=config["computation"]["seed"],
    )

    train_size = max(1, int(len(dataset) * 0.8))
    val_size = max(1, len(dataset) - train_size)
    generator = torch.Generator().manual_seed(config["computation"]["seed"])
    train_subset, val_subset = random_split(dataset, [train_size, val_size], generator=generator)

    loader_kwargs = dict(
        batch_size=config["training"]["batch_size"],
        num_workers=config["computation"]["num_workers"],
        pin_memory=config["computation"]["pin_memory"],
    )
    train_loader = DataLoader(
        train_subset,
        shuffle=True,
        collate_fn=EventEnsembleDataset.collate_fn,
        **loader_kwargs,
    )
    val_loader = DataLoader(
        val_subset,
...
def _collect_ensemble_dirs(config: Dict) -> Dict[str, str]:
    dirs: Dict[str, str] = {}
    for code in config.get("multi_code", {}).get("codes", []):
        section = config.get(code.lower())
        if not section:
            continue
        output_dir = section.get("output_dir")
        if output_dir and Path(output_dir).expanduser().exists():
            dirs[code.lower()] = str(Path(output_dir).expanduser())
    return dirs
...
def main():
    ...
    parser.add_argument(
        '--config',
        default='configs/training/pipeline/default_config.yaml',
    ...
    if args.test_mode:
        ...
        try:
            train_loader, val_loader = create_real_dataloaders(config)
        except Exception as exc:
            logger.error("Failed to build real dataloaders: %s", exc)
            raise

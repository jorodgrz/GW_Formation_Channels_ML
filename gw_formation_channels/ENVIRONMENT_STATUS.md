...
- ✓ `train.py` - All imports working
- ✓ `compas_ensemble/generate_ensemble.py` - All imports working
...
✓ train.py imports successful!

$ python -c "from compas_ensemble.generate_ensemble import *"
...
1. **Start working on your project:**
   ```bash
   source activate_env.sh
   python train.py  # or whatever you want to run
   ```
...
4. **Start training:**
   ```bash
   python train.py
   tensorboard --logdir runs/  # Monitor training in another terminal
   ```

...
**Current Status:** STOPPED LOCALLY – migrating to AWS cluster workflow  
**Next Actions:** Follow `AWS_CLUSTER.md` to redeploy
...
- **Output directory:** `compas_ensemble_sparse/`
...
- **Partial outputs:** moved to `compas_ensemble_sparse_aborted_20251125/` for reference
...
```bash
python compas_ensemble/generate_ensemble.py \
    --sparse \
    --n-systems 10000 \
    --start-index ${START} \
    --end-index ${END} \
    --output-dir /mnt/compas_runs \
    --compas-binary /opt/ASTROTHESIS/COMPAS/src/bin/COMPAS
```
...
### Immediate Actions
- Use AWS infrastructure to run the 40 slices in parallel.
- Once S3 holds the completed ensemble, update this status file with the new metadata location.
- Remove the aborted local directory when no longer needed.

## Files Created

- `compas_ensemble/generate_ensemble.py`: Fixed and working

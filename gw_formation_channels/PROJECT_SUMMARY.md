...
#### 1. COMPAS Ensemble Generation (`compas_ensemble/`)
...
├── compas_ensemble/          # COMPAS ensemble generation (1 file)
├── models/                   # Neural architectures (1 file, 800 lines)
├── inference/                # Bayesian inference (1 file, 500 lines)
├── falsification/            # Testing criteria (1 file, 600 lines)
├── data/                     # Data loading (1 file, 500 lines)
├── utils/                    # Utilities (empty, for future)
├── configs/                  # Configuration (1 YAML)
├── notebooks/                # Jupyter notebooks (for future analysis)
├── train.py                  # Main training (600 lines)
...
Total: ~4500 lines of thoroughly commented Python code
```
...
python -m compas_ensemble.generate_ensemble --n-systems 100000
...
events = GWTC4Loader('gwtc4_data/').load_all_events()
...
```bibtex
@software{gw_formation_channels_2025,

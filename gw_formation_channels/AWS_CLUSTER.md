...
3. Install Miniconda (if preferred) and recreate the `gw_channels` environment:
   ```bash
   conda env create -f /path/to/environment.yml  # or pip install -r requirements.txt
   ```
...
cd /opt/ASTROTHESIS/gw_formation_channels
python compas_ensemble/generate_ensemble.py \
    --sparse \
    --n-systems 10000 \
    --start-index ${START} \
...
Document any completed slices in a shared tracker (S3 JSON, DynamoDB, etc.) so downstream inference modules know which HDF5 files are available.
